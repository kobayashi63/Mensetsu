...
\\
\uncover<2->{\hspace{28pt}\(\cdots \) 非退化}
&
&
\uncover<2->{\hspace{30pt} $\Rightarrow X=0$}
\end{tabular}
\end{table}
...
\hspace{5pt}\uncover<4->{\textcolor<5->{red}{${\small \mathbb{H}^2:=\{ {\bf x}\in\mathbb{E}^3_1\,} {\large \vert} {\small \, \langle {\bf x},{\bf x}\rangle =-1 \} }$}}
}
\end{minipage}
\vspace{7pt}
&
...
\shortstack{
%\textcolor{black}{$\mathbb{S}^2_1$ : pseudo-sphere in $\mathbb{R}^3_1$}
\uncover<3->{\textcolor<5->{red}{${\small \mathbb{S}^2_1:=\{ {\bf x}\in\mathbb{E}^3_1\,} {\large \vert} {\small \, \langle {\bf x},{\bf x}\rangle =1 \} }$}}\\
...
\hspace{10pt}
\uncover<3->{
\scalebox{0.5}{\input{pic_sphere.tex}}
}
\end{minipage}
&
\begin{minipage}[c]{0.15\textwidth}
\hspace{-12pt}
\uncover<5->{\textcolor{red}{$\mathbb{S}^n_s$と$\mathbb{H}^n_{n-s}$は反等長}}
\hspace{-3pt}
\end{minipage}
&
\begin{minipage}[c]{0.4\textwidth}
...
\hspace{45pt}
% \uncover<2->{
\textcolor{red}{形作用素が対角化不可能, 平均曲率とスカラー曲率が一定}
  % }
\vspace{3pt} 
\\
...
\begin{itemize}
\item \textcolor<3->{red}{B-scroll}またはcomplex circleの擬双曲的ガウス写像
%\item 平行曲面の擬双曲的ガウス写像
...
\uncover<3->{
{\large \underline{\, Part 2 \, \(\cdots \) generalizations of \textcolor{red}{B-scroll} in \(\tilde{M}^{m}_s\)}}\vspace{2pt}
\begin{itemize}
\item generalized umbilical hypersurface in \(\tilde{M}^{n+1}_1\) \, \uncover<4->{\textcolor{red}{\(\cdots\) 次元一般化}}
\item generalized umbilical hypersurface in \(\tilde{M}^{n+1}_2\) \, \uncover<4->{\textcolor{red}{\(\cdots\) 指数及び次元一般化}}
\item generalized B-scroll\, in \, \(\mathbb{S}^5_2\)\, or \, \(\mathbb{H}^5_2\) \, \uncover<4->{\textcolor{red}{\(\cdots\) 指数及び余次元一般化}}
\end{itemize}
}
}
  ...
  \hspace{-10pt}
   \begin{tabular}{cl}
     1970年代 & ・\(\Delta H=\lambda H\)となるリーマン部分多様体のtype number\\
          & ・null \(2\)-typeかつ \(H\) : const\, なローレンツ曲面の完全分類\\
          & \\
     1980年代 & ・部分多様体のガウス写像のtype number\\
          & \\
...
\(\tilde{M}^{n+1}_1 = \mathbb{E}^{n+1}_1\) or \(\mathbb{S}^{n+1}_1\) or \(\mathbb{H}^{n+1}_1\)
\vspace{3pt}

\(M^n_1\) \hspace{9pt}: \(\tilde{M}^{n+1}_1\)のローレンツ超曲面，isoparametric
\vspace{3pt}
...
\uncover<1->{\hspace{200pt}{\small 別証明}}
\vspace{-8pt}
...
\(
    \left\{
  \begin{aligned}
    & \text{\, Type (\rnum {1}) \hspace{-0pt} : Cartan frame field \uncover<1->{\hspace{37pt} {\small \(\cdots\) H. Kobayashi}}}\\
    & \text{\, Type (\rnum {2}) : bi-null Cartan frame field \uncover<1->{\hspace{7pt} {\small \(\cdots\) M. Sakaki--A. U\c{c}um--K. \.{I}larslan}}}\\
    & \text{\, Type (\rnum {3}) \uncover<1->{\hspace{128pt} {\small \(\cdots \,m=5\)のとき，H. Kobayashi}}}
  \end{aligned}
    \right.
  \)
}
...
\frame{
  \frametitle{\insertsection \quad \quad{\normalsize $\cdots$ \insertsubsection}}
\begin{alertblock}{Main Result 5 (H. Kobayashi)}
      {\it 
      % \hspace{30pt}
    ...
    \begin{center}
      \begin{minipage}{0.7\textwidth}
    \begin{enumerate}
    \item \(Z_1\) : non-null  \hspace{5pt}$\Rightarrow$\hspace{5pt}  \( P(x)=(x-(\textcolor<4->{red}{\varepsilon_C k_2^2+\varepsilon_1 k_3^2}) )^2\) \\
    \item \(Z_1\) : null  \hspace{26pt}$\Rightarrow$ \hspace{2pt} \( P(x)=(x-\textcolor<4->{red}{k_2^2})^2\)
    \end{enumerate}
  \end{minipage}
    \end{center}
      %\vspace{-2pt}
...
%\underline{{\footnotesize {\bf Case 3} }}
\vspace{-3pt}
\begin{block}{{\small Type (\rnum{3})\, (H. Kobayashi)}}
  \vspace{-8pt}
{\footnotesize
% \begin{eqnarray}\label{joken6}
% {\renewcommand\arraystretch{0.3}
